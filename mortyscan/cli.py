"""Командная строка MortyScan v18.2 «Инквизитор».
...
[yellow]Что умею:[/yellow]
  ...
  • Проверять новые версии и обновляться почти в один клик
"""
...
@app.command("update")
def update_cmd(
    yes: bool = typer.Option(False, "--yes", "-y", help="Сразу запускать обновление без дополнительного вопроса."),
):
    """Проверить наличие новой версии и при необходимости скачать обновление."""
    if maybe_offer_update(interactive=True, auto_accept=yes):
        raise typer.Exit(0)
    console.print("[green]Обновлений не найдено.[/green]")
