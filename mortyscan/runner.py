...
async def run_scan(target: str, auth: Authorization, *,
    ...
    console.rule(f"[bold magenta]MortyScan v18.1 → {host} ({ctx.ip or 'не разрешено'})")

    perms_human = [
        ("активные пробы", "ДА" if ctx.permissions["intrusive"] else "нет"),
        ("стресс-тест", "ДА" if ctx.permissions["stress"] else "нет"),
        ("ARP-скан LAN", "ДА" if ctx.permissions["local_arp"] else "нет"),
    ]
            ...
            for name, mod, human in ALL_MODULES:
                ...
                try:
                    ...
                    if LOG.isEnabledFor(logging.DEBUG):
                        ...
                        LOG.error("модуль %s упал: %s", name, e)
                    console.print(f"[yellow]модуль {name} пропущен из-за ошибки:[/yellow] {e}")
